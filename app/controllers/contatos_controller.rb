class ContatosController < ApplicationController
    # Executa o método privado autenticar antes de executar qualquer outra ação.
    ...
    def create
        ...
        # Salva o contato no banco de dados. A função save retorna um booleano indicando se o salvamento foi feito com sucesso.
        if @contato.save
    ...
    def show
        ...
        if @contato.usuario_id == @usuario.id
            # Se pertencer, responde com o JSON do contato.
            render json: @contato
        else
            # Se não pertencer, responde com o seguinte JSON e o status 404 (Não encontrado).
            render json: { erro: "Não encontrado" }, status: :not_found
        end
    end

    def index
        # Busca no banco de dados todos os contatos que pertencem ao @usuario.
    ...
    def update
        ...
        if @contato.usuario_id == @usuario.id
            ...
            # Caso contrário, responde a requisição com o seguinte JSON e status 404 (Não encontrado).
            render json: { erro: "Não encontrado." }, status: :not_found
        end
    end
